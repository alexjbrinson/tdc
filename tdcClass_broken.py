...
class TimeStampTDC1(object):
    ...
    def __init__(
        ...
        try:
            ...
            self.EPICS_CA_AUTO_ADDR_LIST="NO"
            os.environ["EPICS_CA_ADDR_LIST"] = "10.54.0.1"
            self.server = SimpleServer()
        except: print('wahuh fuk m8')
        """
    ...
    def toHist(self, buffer):
      ...
      for channel in self.remainder.keys():
        ...
        if len(eventTimes)>0 and len(triggers)>0:
            ...
            if self.useEpics:
              print("caputting data for channel",channel)
              print(type(channel))
              chn=int(channel.strip('channel ')); print('test:chn=',chn)
              caput("Beamline:counts%d"%chn, binIncrements)
    ...
    def start_continuous_stream_timestamps_to_file(self, filename: str, cleanDBname: str, run:int, binRay=[], pickleDic="iTurnedMyselfIntoAPickle.pkl", timeStreamFile='timeStreamLiveData.pkl', tStreamLength=100, useEpics=True):
        """
        ...
        if useEpics:#cry
            self.useEpics=True
            prefix='Beamline:'
            pvdb1 = {"counts1":{"prec" : 0, "units":"none", 'count':binRay[2] if len(binRay)==3 else 1}}
            pvdb2 = {"counts2":{"prec" : 0, "units":"none", 'count':binRay[2] if len(binRay)==3 else 1}}
            pvdb3 = {"counts3":{"prec" : 0, "units":"none", 'count':binRay[2] if len(binRay)==3 else 1}}
            pvdb4 = {"counts4":{"prec" : 0, "units":"none", 'count':binRay[2] if len(binRay)==3 else 1}}
            self.server.createPV(prefix, pvdb1)
            self.server.createPV(prefix, pvdb2)
            self.server.createPV(prefix, pvdb3)
            self.server.createPV(prefix, pvdb4)
