...
class TimeStampTDC1(object):
    ...
    def _continuous_stream_timestamps_to_database(self, dbname: str, run: int):
        """
        Indefinitely streams timestamps to a file
        WARNING: ensure there is sufficient disk space: 32 bits x total events required
        """
        ...
        cmd_str = "INPKT;{} {};time {};timestamp;counts?;".format(level_str, level, 0)
        self._com.write((cmd_str + "\r\n").encode())
        sql = 'INSERT OR IGNORE INTO TDC (tStamp, channel, run) values(?, ?, ?)' #SQL command template
        while self.accumulate_timestamps:
            buffer = self._com.read((1 << 20) * 4)
            print(buffer)
            (times,channels) = self.read_timestamps_bin(buffer)
            times=[int(t) for t in times]
            channels = [int(ch, 2) for ch in channels]
            runs=[run for t in times]
            data = list(zip(times,channels, runs))
            #print(data)
            self.db.executemany(sql,data) #writing to database!
            self.db.commit()
            #with open(filename, "ab+") as f:
            #    f.write(buffer)
            #f.close()
    ...
    def start_continuous_stream_timestamps_to_database(self, dbname, run):
        """
        Starts the timestamp streaming service to file in the brackground
        """
        self.accumulate_timestamps = True
        self.procdb = threading.Thread(
            target=self._continuous_stream_timestamps_to_database,
            args=(dbname,run,),
        )
        self.procdb.daemon = True  # Daemonize thread
        self.procdb.start()  # Start the execution

    def stop_continuous_stream_timestamps_to_database(self):
        """
        Stops the timestamp streaming service to file in the brackground
        """
        self.accumulate_timestamps = False
